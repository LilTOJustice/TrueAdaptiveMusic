...
class PackListWidget(
    ...
    open inner class Entry(private val musicPack: MusicPack?): ObjectSelectionList.Entry<Entry>() {
        ...
        override fun renderContent(
            ...
            context.textRenderer().acceptScrolling(
        ...
        private fun renderPackImage(context: GuiGraphics) {
            musicPack ?: return
            val identifier = Identifier.fromNamespaceAndPath(
                "trueadaptivemusic",
                "icon/" +
                        Util.sanitizeName(musicPack.packName, Identifier::validPathChar)
            )
                    ...
                    .use { stream ->
                        stream?.let { stream ->
                            minecraft.textureManager.register(
                                identifier,
                                DynamicTexture(
                                    identifier::toString, NativeImage.read(stream))
                            )
                        }
