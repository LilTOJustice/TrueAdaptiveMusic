...
class EditPackScreen(private val parent: Screen, private val musicPack: MusicPack)
    ...
    private lateinit var saveButtonWidget: TextAndImageButton
    ...
    override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
        val optional = this.getChildAt(mouseX, mouseY)
        if (optional.isEmpty) {
            return false
        } else {
    ...
    override fun init() {
        ...
        val saveButtonWidgetWidth = font.width(SAVE_BUTTON_TEXT) + 20

        saveButtonWidget = TextAndImageButton.Builder(SAVE_BUTTON_TEXT, CHECKMARK) {
            exportAndClose()
        }
            .usedTextureSize(9, 8)
            .textureSize(9, 8)
            .offset((saveButtonWidgetWidth - 20) / 2, 6)
            .build()
    ...
    private fun positionContainers() {
        ...
        packStructureWidget.setHeight1(getContainerHeight())
        nodeViewWidget.setHeight1(getContainerHeight())
        predicateViewWidget.setHeight1(getContainerHeight())
        eventViewWidget.setHeight1(getContainerHeight())
        ...
        if (predicateView) {
            ...
            nodeViewWidget.setHeight1(getContainerHeight() / 2)
            eventViewWidget.setHeight1(getContainerHeight() / 2)
