...
object CombatPredicate: PredicateType<CombatPredicate.Arguments, CombatPredicate.State>(
    ...
    class State(private val arguments: Arguments): TriggerState() {
        ...
        fun test(minecraft: Minecraft): Boolean {
            ...
            val horizontalFov = 2 * atan(tan(verticalFov / 2) * minecraft.window.screenWidth / minecraft.window.screenHeight)
            val verticalAngle = acos(playerEntity.lookAngle.y)
            val horizontalAngle = acos(playerEntity.lookAngle.x)
    ...
    private fun isValidAttacker(entity: LivingEntity, playerEntity: Player, displacement: Vec3): Boolean {
        ...
        return closeEnough && (
                (entity as? Mob)?.isAggressive == true ||
                        ...
                        (entity as? ElderGuardian)?.let { it.target?.id == playerEntity.id } == true ||
                        entity is Phantom ||
    ...
    private fun closeEnough(displacement: Vec3, attackerSize: Vec3): Boolean
    {
        ...
        val scaledAttackerMinDistance = baseAxialDistance
            .multiply(
                Vec3(cbrt(attackerSize.x), cbrt(attackerSize.y), cbrt(attackerSize.z)))
