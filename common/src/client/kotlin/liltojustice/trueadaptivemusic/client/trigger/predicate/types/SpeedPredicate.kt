...
object SpeedPredicate: StaticPredicateType<SpeedPredicate.Arguments>(
    ...
    override fun test(arguments: Arguments): Boolean {
        ...
        val currentSpeedSqr = when (arguments.speedType) {
            SpeedType.All -> player.knownMovement.scale(tickRate).distanceToSqr(Vec3.ZERO)
            SpeedType.Horizontal -> player.knownMovement.horizontal().scale(tickRate).distanceToSqr(Vec3.ZERO)
            SpeedType.Vertical -> player.knownMovement.y * player.knownMovement.y * tickRate
        }
