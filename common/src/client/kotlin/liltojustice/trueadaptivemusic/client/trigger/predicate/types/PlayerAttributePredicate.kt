...
object PlayerAttributePredicate: StaticPredicateType<PlayerAttributePredicate.Arguments>(
    ...
    override fun test(arguments: Arguments): Boolean {
        ...
        val registry = player.commandSenderWorld.registryAccess().registry(Registries.ATTRIBUTE).getOrNull()
            ?: return false
        val entityAttributeEntry =
            registry
                .mapNotNull { registry.wrapAsHolder(it) }
                .firstOrNull { entityAttributeEntry ->
                    entityAttributeEntry.`is`(arguments.attribute.id)
                }
        val entityAttributeValue = entityAttributeEntry?.let {
            if (player.attributes.hasAttribute(entityAttributeEntry))
                player.attributes.getValue(entityAttributeEntry.value())
            else
                return false
        } ?: return false
