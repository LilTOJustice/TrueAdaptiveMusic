...
abstract class ContainerWidget(
    ...
    override fun mouseScrolled(mouseX: Double, mouseY: Double, verticalAmount: Double): Boolean {
        ...
        children.forEach { (_, child) ->
            if (child.widget.isMouseOver(mouseX, mouseY)) {
                child.widget.mouseScrolled(mouseX, mouseY, verticalAmount)
    ...
    private fun drawVerticalScrollbar(context: DrawContext?): Triple<Int, Int, Int>? {
        ...
        context?.let {
            it.setShaderColor(0f, 0f, 0f, 0.75f)
            it.drawVerticalLine(x, y + headerOffset, y + adjustedHeight + headerOffset, Colors.BLACK)
            it.setShaderColor(1f, 1f, 1f, 1f)
            it.drawVerticalLine(
                x,
                extent.first,
                extent.second,
    ...
    private fun drawHorizontalScrollbar(context: DrawContext?): Triple<Int, Int, Int>? {
        ...
        context?.let {
            it.setShaderColor(0f, 0f, 0f, 0.75f)
            it.drawHorizontalLine(x + offset, x + usableWidth, y, Colors.BLACK)
            it.setShaderColor(1f, 1f, 1f, 1f)
