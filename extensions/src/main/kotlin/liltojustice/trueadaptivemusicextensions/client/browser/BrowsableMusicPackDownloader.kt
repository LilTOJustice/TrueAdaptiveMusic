...
import liltojustice.trueadaptivemusicextensions.client.serialization.EnumTypeAdapter
...
object BrowsableMusicPackDownloader {
    ...
    suspend fun fetchPacksFromRepository(ignoreCache: Boolean): PackManifest? {
        ...
        if (!ignoreCache && ExtensionConstants.MANIFEST_PATH.exists()) {
            return gson
                .fromJson(ExtensionConstants.MANIFEST_PATH.toFile().readText(), PackManifest::class.java)
        }

        coroutineScope { CurlHelper.get(MANIFEST_FILE_URL, ExtensionConstants.MANIFEST_PATH_TEMP) }

        if (!ExtensionConstants.MANIFEST_PATH_TEMP.exists()) {
        ...
        val manifest = try {
            gson
                .fromJson(ExtensionConstants.MANIFEST_PATH_TEMP.toFile().readText(), PackManifest::class.java)
        ...
        ExtensionConstants.MANIFEST_PATH_TEMP.moveTo(ExtensionConstants.MANIFEST_PATH, true)
        ExtensionConstants.MANIFEST_PATH.toFile().writeText(gson.toJson(manifest))
