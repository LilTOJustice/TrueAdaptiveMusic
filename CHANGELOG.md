True Adaptive Music v1.6.1

Features:
- Volume boost setting - **FFmpeg required!**
  - In the True Adaptive Music options, you can add a loudness boost to the music/ambience tracks

Fixes/Changes:
- Fixed an issue where the vanilla sound engine would break if a pack was selected after launching with no pack selected
- Filled out MOST missing Russian translations. More coming in the next release.
- Improved some of the fading logic and increased how long tracks fade in/out
- Updated the mod menu icon
- Mod menu options now points to the main True Adaptive Music screen, instead of the options screen
- Mono audio files will now properly play in your ears rather than at the world origin

[1.7](https://github.com/LilTOJustice/TrueAdaptiveMusic/milestone/8) is starting development soon